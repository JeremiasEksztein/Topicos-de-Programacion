...
int vectorLeerDeTexto(Vector_t* vector, const char* nomArch, int (ParsearTexto)(char*, void*))
{
    ...
    while(fgets(buffer, TAM_LINEA_BUFFER, arch)){
        if((err = ParsearTexto(buffer, tmp)) != EXITO){
            fclose(arch);
            free(tmp);
            return err;
        }
