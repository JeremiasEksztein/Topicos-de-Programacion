...
int campoValidar(Campo* c)
{
	...
	case CAMPO_TIPO_NUMERICO:; /* C considera un error declarar una variable debajo de un label, ; es un statement vacio*/
		...
		if (!stringLenght(c->respuesta)) {
			printf(COLOR_BOLD COLOR_RED "Tiene que escribir en el campo" COLOR_RESET);
			esperarInput();
			return 0;
		}

		if ((tmpF = atof(c->respuesta)) < atn->min || tmpF > atn->max) {
			printf(COLOR_BOLD COLOR_RED "Respuesta fuera del rango valido [%lf : %lf]" COLOR_RESET, atn->min, atn->max);
			esperarInput();
			return 0;
		}
	...
	case CAMPO_TIPO_TEXTO:;
		...
		if (stringLenght(c->respuesta) > att->len) {
			printf(COLOR_BOLD COLOR_RED "Respuesta demasiado larga, maxlen: %d" COLOR_RESET, att->len);
			esperarInput();
			return 0;
		}
...
int formularioPublicar(Formulario_t* f)
{
	...
	/*Esta cantidad de ciclo anidados es necesaria
		la alternativa sino seria usar un nido de goto's
	*/

	do { /* Ciclo exterior - Esta conforme el usuario con su respuesta? */
		do { /* Ciclo interior - Son las respuestas del formulario validas?*/
			esperarInput();
			fflush(stdout);

			for (i = 0; i < f->ce; i++) {
				do { /* Ciclo interior - Son las respuestas campo, por campo, validas?*/
					puts(FLUSH_TERMINAL);
					printf(COLOR_BOLD "---------" COLOR_YELLOW "%s" COLOR_GRAY COLOR_BOLD "---------\n" COLOR_RESET, f->titulo);
					c = (f->campos + i);
		...
		puts("Su seleccion: "); /* Mostramos lo que el usuario selecciono */
		for (i = 0; i < f->ce; i++) {
			c = (f->campos + i);
			printf("%0d - " COLOR_BOLD COLOR_CYAN "%s" COLOR_RESET "\n", i + 1, c->respuesta);
		}

		do { /* Hasta que no confirme el usuario si esta o no conforme */
